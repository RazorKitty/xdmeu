...
drawmenu(void)
{
	...
	if (passwd) {
	    censored = ecalloc(1, sizeof(text));
        memset(censored, censor, strlen(text));
        drw_text(drw, x, 0, w, bh, lrpad / 2, censored, 0);
	} else
...
readxresources(void) {
	...
	if ((xrm = XResourceManagerString(dpy))) {
        ...
        if (XrmGetResource(xdb, "dmenu.censor", "*", &type, &xval))
            memcpy(&censor, xval.addr, 1);
